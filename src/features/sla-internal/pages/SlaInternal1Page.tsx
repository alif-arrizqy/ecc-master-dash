...
const COLS: { key: keyof Sla1AggregateRow | string; label: string }[] = [
  ...
  { key: 'unknown_time', label: 'UNKNOWN TIME' },
  { key: 'up_persentase', label: 'UP PERSENTASE' },
  { key: 'unknown_persentase', label: 'UNKNOWN PERSENTASE' },
  { key: 'eh1', label: 'EH1' },
  { key: 'eh2', label: 'EH2' },
  { key: 'eh3', label: 'EH3' },
...
const SlaInternal1Page = () => {
  ...
  const [rows, setRows] = useState<Sla1AggregateRow[]>([]);
  ...
  const runQuery = async () => {
    ...
    try {
      ...
      setRows(results);
      toast.success(`SLA 1 dimuat: ${results.length} site`);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Gagal memuat SLA 1');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const exportExcel = () => {
    ...
    const plain = rows.map((r) => {
      ...
      for (const c of COLS) {
        o[c.key] = r[c.key as keyof Sla1AggregateRow];
      }
      return o;
    });
    downloadAoAsExcel(
      ...
      'SLA1',
      `sla-internal-1-${new Date().toISOString().slice(0, 10)}.xlsx`
    );
        ...
        <CardContent className="p-0">
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  slice.map((row, i) => (
                    <TableRow key={`${row.nojs}-${row.site}-${i}`} className="hover:bg-muted/30">
                      {COLS.map((c) => (
                        <TableCell key={c.key} className="text-xs whitespace-nowrap max-w-[220px] truncate">
                          {String(row[c.key as keyof Sla1AggregateRow] ?? '')}
                        </TableCell>
                      ))}
                    </TableRow>
                  ))
          ...
          {rows.length > 0 && (
            <div className="flex items-center justify-between p-4 border-t border-border">
              <p className="text-sm text-muted-foreground">Halaman {pageSafe} dari {totalPages}</p>
...
export default SlaInternal1Page;
